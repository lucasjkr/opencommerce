<?php
class ModelSettingStoreAdmin extends Model {
	public function addStore($data) {
		$this->db->query("INSERT INTO oc_store SET name = '" . $this->db->escape((string)$data['config_name']) . "', `url` = '" . $this->db->escape((string)$data['config_url']) . "', `ssl` = '" . $this->db->escape((string)$data['config_ssl']) . "'");
		...
		foreach ($query->rows as $layout_route) {
			$this->db->query("INSERT INTO oc_layout_route SET layout_id = '" . (int)$layout_route['layout_id'] . "', route = '" . $this->db->escape($layout_route['route']) . "', store_id = '" . (int)$store_id . "'");
		}
	...
	public function editStore($store_id, $data) {
		$this->db->query("UPDATE oc_store SET name = '" . $this->db->escape((string)$data['config_name']) . "', `url` = '" . $this->db->escape((string)$data['config_url']) . "', `ssl` = '" . $this->db->escape((string)$data['config_ssl']) . "' WHERE store_id = '" . (int)$store_id . "'");

		$this->cache->delete('store');
	}

	public function deleteStore($store_id) {
		$this->db->query("DELETE FROM oc_store WHERE store_id = '" . (int)$store_id . "'");
		$this->db->query("DELETE FROM oc_layout_route WHERE store_id = '" . (int)$store_id . "'");

		$this->cache->delete('store');
	}

	public function getStore($store_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM oc_store WHERE store_id = '" . (int)$store_id . "'");
	...
	public function getTotalStoresByLayoutId($layout_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM oc_setting WHERE `key` = 'config_layout_id' AND `value` = '" . (int)$layout_id . "' AND store_id != '0'");
