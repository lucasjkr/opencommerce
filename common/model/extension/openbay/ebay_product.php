...
class ModelExtensionOpenBayEbayProduct extends Model {
	...
	public function importItems($data) {
		...
		if ($options['cat'] == 1 || !isset($options['cat'])) {
			...
			foreach ($categories as $key1 => $cat1) {
				foreach ($cat1 as $key2 => $cat2) {
					...
					$qry = $this->db->query("SELECT * FROM `oc_category`, `oc_category_description` WHERE `oc_category`.`parent_id` = '0' AND `oc_category_description`.`name` = '" . $this->db->escape($key2) . "' LIMIT 1");

					if ($qry->num_rows != 0) {
						$id1 = $qry->row['category_id'];
					} else {
						$this->db->query("INSERT INTO `oc_category` SET `parent_id` = '0', `status` = '1', `top` = '1'");
						$id1 = $this->db->getLastId();
						$this->db->query("INSERT INTO `oc_category_description` SET `name` = '" . $this->db->escape($key2) . "', `language_id` = '" . (int)$this->config->get('config_language_id') . "', `category_id` = '" . $this->db->escape($id1) . "'");
						$this->db->query("INSERT INTO `oc_category_to_store` SET `category_id` = '" . $this->db->escape($id1) . "', `store_id` = '0'");
					}

					if (!empty($cat2)) {
						foreach ($cat2 as $key3 => $cat3) {
							$qry = $this->db->query("SELECT * FROM `oc_category`, `oc_category_description` WHERE `oc_category`.`parent_id` = '" . $this->db->escape($id1) . "' AND `oc_category_description`.`name` = '" . $this->db->escape($key3) . "' LIMIT 1");

							if ($qry->num_rows != 0) {
								$id2 = $qry->row['category_id'];
							} else {
								$this->db->query("INSERT INTO `oc_category` SET `parent_id` = '" . $this->db->escape($id1) . "', `status` = '1', `top` = '1'");
								$id2 = $this->db->getLastId();
								$this->db->query("INSERT INTO `oc_category_description` SET `name` = '" . $this->db->escape($key3) . "', `language_id` = '" . (int)$this->config->get('config_language_id') . "', `category_id` = '" . $this->db->escape($id2) . "'");
								$this->db->query("INSERT INTO `oc_category_to_store` SET `category_id` = '" . $this->db->escape($id2) . "', `store_id` = '0'");
							}

							if (!empty($cat3)) {
								foreach ($cat3 as $key4 => $cat4) {
									$qry = $this->db->query("SELECT * FROM `oc_category`, `oc_category_description` WHERE `oc_category`.`parent_id` = '" . $this->db->escape($id2) . "' AND `oc_category_description`.`name` = '" . $this->db->escape($key4) . "' LIMIT 1");

									if ($qry->num_rows != 0) {
										$id3 = $qry->row['category_id'];
									} else {
										$this->db->query("INSERT INTO `oc_category` SET `parent_id` = '" . $this->db->escape($id2) . "', `status` = '1', `top` = '1'");
										$id3 = $this->db->getLastId();
										$this->db->query("INSERT INTO `oc_category_description` SET `name` = '" . $this->db->escape($key4) . "', `language_id` = '" . (int)$this->config->get('config_language_id') . "', `category_id` = '" . $id3 . "'");
										$this->db->query("INSERT INTO `oc_category_to_store` SET `category_id` = '" . $this->db->escape($id3) . "', `store_id` = '0'");
									}

									if (!empty($cat4)) {
										foreach ($cat4 as $key5 => $cat5) {
											$qry = $this->db->query("SELECT * FROM `oc_category`, `oc_category_description` WHERE `oc_category`.`parent_id` = '" . $this->db->escape($id3) . "' AND `oc_category_description`.`name` = '" . $this->db->escape($key5) . "' LIMIT 1");

											if ($qry->num_rows != 0) {
												$id4 = $qry->row['category_id'];
											} else {
												$this->db->query("INSERT INTO `oc_category` SET `parent_id` = '" . $this->db->escape($id3) . "', `status` = '1', `top` = '1'");
												$id4 = $this->db->getLastId();
												$this->db->query("INSERT INTO `oc_category_description` SET `name` = '" . $this->db->escape($key5) . "', `language_id` = '" . (int)$this->config->get('config_language_id') . "', `category_id` = '" . $this->db->escape($id4) . "'");
												$this->db->query("INSERT INTO `oc_category_to_store` SET `category_id` = '" . $this->db->escape($id4) . "', `store_id` = '0'");
											}
		...
		foreach ($new_data_1 as $item) {
			if (!in_array($item['ItemID'], $current)) {
				...
				$this->db->query("INSERT INTO `oc_product_description` SET `product_id` = '" . (int)$product_id . "', `language_id` = '" . (int)$this->config->get('config_language_id') . "', `name` = '" . $this->db->escape(htmlspecialchars(base64_decode($item['Title']), ENT_COMPAT, 'UTF-8')) . "', `description` = '" . $this->db->escape(htmlspecialchars(utf8_encode($item['Description']), ENT_COMPAT, 'UTF-8')) . "'");
	...
	private function createVariants($product_id, $data) {
		foreach ($data['variation']['vars'] as $variant_id => $variant) {
			...
			$this->db->query("UPDATE `oc_product_option_value` SET subtract = '0', price = '0.000', quantity = '0' WHERE product_id = '" . (int)$product_id . "'");
		}

		$all_variants = $this->model_extension_module_openstock->calculateVariants($product_id);
		foreach ($all_variants as $new_variant) {
			$this->db->query("INSERT INTO `oc_product_option_variant` SET `product_id` = '" . (int)$product_id . "', `sku` = '', `stock` = '0', `active` = '0', `subtract` = '1', `price` = '0.00', `image` = '', `weight` = '0.00'");

			$variant_id = $this->db->getLastId();

			$i = 1;
			foreach ($new_variant as $new_variant_value) {
				$this->db->query("INSERT INTO `oc_product_option_variant_value` SET `product_option_variant_id` = '" . (int)$variant_id . "', `product_option_value_id` = '" . (int)$new_variant_value . "', `product_id` = '" . (int)$product_id . "', `sort_order` = '" . (int)$i++ . "'");
			}
		}
		...
		foreach ($data['variation']['vars'] as $variant_id => $variant) {
			...
			if (!empty($variant_row)) {
				$this->db->query("UPDATE `oc_product_option_variant` SET `product_id` = '" . (int)$product_id . "', `sku` = '" . $this->db->escape($variant['sku']) . "', `stock` = '" . (int)$variant['qty'] . "', `active` = 1, `price` = '" . (float)$variant['price'] . "' WHERE `product_option_variant_id` = '" . (int)$variant_row['product_option_variant_id'] . "'");
			}
		}
	}

	private function getOption($name) {
		$qry = $this->db->query("SELECT * FROM `oc_option` `o` LEFT JOIN `oc_option_description` `od` ON (`od`.`option_id` = `o`.`option_id`) WHERE `od`.`name` = '" . $this->db->escape($name) . "'LIMIT 1");
	...
	private function createOption($name) {
		$this->db->query("INSERT INTO `oc_option` SET `type` = 'select', `sort_order` = IFNULL((select `sort` FROM (SELECT (MAX(`sort_order`)+1) AS `sort` FROM `oc_option`) AS `i`),0)");

		$option_id = $this->db->getLastId();

		$qry_sort = $this->db->query("SELECT * FROM `oc_option` WHERE `option_id` = '" . (int)$option_id . "' LIMIT 1");

		$this->db->query("INSERT INTO `oc_option_description` SET `language_id` = '" . (int)$this->config->get('config_language_id') . "', `name` = '" . $this->db->escape($name) . "', `option_id` = '" . (int)$option_id . "'");

		return array('id' => (int)$option_id, 'sort' => (int)$qry_sort->row['sort_order']);
	}

	private function getOptionValue($name, $option_id) {
		$qry = $this->db->query("SELECT * FROM `oc_option_value` ov LEFT JOIN `oc_option_value_description` `ovd` ON (`ovd`.`option_value_id` = `ov`.`option_value_id`) WHERE `ovd`.`name` = '" . $this->db->escape($name) . "' AND `ovd`.`option_id` = '" . (int)$option_id . "'LIMIT 1");
	...
	private function createOptionValue($name, $option_id) {
		$this->db->query("INSERT INTO `oc_option_value` SET `option_id` = '" . (int)$option_id . "', `sort_order` = IFNULL((select `sort` FROM (SELECT (MAX(`sort_order`)+1) AS `sort` FROM `oc_option_value`) AS `i`),0)");

		$id = $this->db->getLastId();

		$this->db->query("INSERT INTO `oc_option_value_description` SET `language_id` = '" . (int)$this->config->get('config_language_id') . "', `name` = '" . $this->db->escape($name) . "', `option_id` = '" . (int)$option_id . "', `option_value_id` = '" . (int)$id . "'");

		return array('id' => (int)$id);
	}

	private function getProductOption($product_id, $option_id) {
		$qry = $this->db->query("SELECT * FROM  oc_product_option WHERE product_id = '" . (int)$product_id . "' AND option_id = '" . (int)$option_id . "' LIMIT 1");
	...
	private function getProductOptionValue($product_id, $option_id, $option_value_id, $product_option_id) {
		$qry = $this->db->query("SELECT * FROM  `oc_product_option_value` WHERE `product_id` = '" . (int)$product_id . "' AND `option_id` = '" . (int)$option_id . "' AND `product_option_id` = '" . (int)$product_option_id . "' AND `option_value_id` = '" . (int)$option_value_id . "' LIMIT 1");

		if ($qry->num_rows != 0) {
			return $qry->row['product_option_value_id'];
		} else {
			$this->db->query("INSERT INTO oc_product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$option_id . "', option_value_id = '" . (int)$option_value_id . "'");
	...
	private function attributeGroupExists($name) {
		...
		$qry = $this->db->query("SELECT * FROM  `oc_attribute_group_description` WHERE `name` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "' AND `language_id` = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");

		if ($qry->num_rows != 0) {
			return $qry->row['attribute_group_id'];
		} else {
			$this->openbay->ebay->log('New group');
			$qry2 = $this->db->query("SELECT `sort_order` FROM  `oc_attribute_group` ORDER BY `sort_order` DESC LIMIT 1");
			...
			$this->db->query("INSERT INTO `oc_attribute_group` SET `sort_order` = '" . (int)$sort . "'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `oc_attribute_group_description` SET `attribute_group_id` = '" . (int)$id . "', `language_id` = '" . (int)$this->config->get('config_language_id') . "', `name` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "'");
	...
	private function attributeExists($group_id, $name) {
		...
		$qry = $this->db->query("SELECT * FROM `oc_attribute_description` `ad`, `oc_attribute` `a` WHERE `ad`.`name` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "' AND `ad`.`language_id` = '" . (int)$this->config->get('config_language_id') . "' AND `a`.`attribute_id` = `ad`.`attribute_id` AND `a`.`attribute_group_id` = '" . (int)$group_id . "' LIMIT 1");

		if ($qry->num_rows != 0) {
			return $qry->row['attribute_id'];
		} else {
			$this->openbay->ebay->log('New attribute');
			$qry2 = $this->db->query("SELECT `sort_order` FROM  `oc_attribute` ORDER BY `sort_order` DESC LIMIT 1");
			...
			$this->db->query("INSERT INTO `oc_attribute` SET `sort_order` = '" . (int)$sort . "', `attribute_group_id` = '" . (int)$group_id . "'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `oc_attribute_description` SET `attribute_id` = '" . (int)$id . "', `language_id` = '" . (int)$this->config->get('config_language_id') . "', `name` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "'");
	...
	private function attributeAdd($product_id, $attribute_id, $name) {
		...
		$sql = $this->db->query("SELECT * FROM `oc_product_attribute` WHERE `product_id` = '" . (int)$product_id . "' AND `attribute_id` = '" . (int)$attribute_id . "' AND `language_id` = '" . (int)$this->config->get('config_language_id') . "'");

		if ($sql->num_rows == 0) {
			$this->db->query("INSERT INTO `oc_product_attribute` SET `product_id` = '" . (int)$product_id . "', `attribute_id` = '" . (int)$attribute_id . "', `text` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "', `language_id` = '" . (int)$this->config->get('config_language_id') . "'");
		}
	}
	...
	private function manufacturerExists($name) {
		...
		$qry = $this->db->query("SELECT * FROM  `oc_manufacturer` WHERE LCASE(`name`) = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "' LIMIT 1");

		if ($qry->num_rows != 0) {
			return $qry->row['manufacturer_id'];
		} else {
			$this->openbay->ebay->log('New manufacturer');
			$qry2 = $this->db->query("SELECT `sort_order` FROM  `oc_manufacturer` ORDER BY `sort_order` DESC LIMIT 1");
			...
			$this->db->query("INSERT INTO `oc_manufacturer` SET `sort_order` = '" . (int)$sort . "', `name` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `oc_manufacturer_to_store` SET `manufacturer_id` = '" . (int)$id . "', `store_id` = '0'");
	...
	private function weightClassExists($name) {
		...
		$qry = $this->db->query("SELECT `weight_class_id` FROM `oc_weight_class_description` WHERE LCASE(`title`) = '" . $this->db->escape(strtolower($name)) . "' LIMIT 1");
		...
		if ($qry->num_rows != 0) {
			...
			$this->db->query("INSERT INTO `oc_weight_class` SET `value` = '1'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `oc_weight_class_description` SET `language_id` = '" . (int)$this->config->get('config_language_id') . "', `weight_class_id` = '" . (int)$id . "', `title` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "', `unit` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "'");
	...
	private function lengthClassExists($name) {
		...
		$qry = $this->db->query("SELECT `length_class_id` FROM `oc_length_class_description` WHERE LCASE(`title`) = '" . $this->db->escape(strtolower($name)) . "' LIMIT 1");
		...
		if ($qry->num_rows != 0) {
			...
			$this->db->query("INSERT INTO `oc_length_class` SET `value` = '1'");

			$id = $this->db->getLastId();

			$this->db->query("INSERT INTO `oc_length_class_description` SET `language_id` = '" . (int)$this->config->get('config_language_id') . "', `length_class_id` = '" . (int)$id . "', `title` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "', `unit` = '" . $this->db->escape(htmlspecialchars($name, ENT_COMPAT)) . "'");
	...
	private function addImage($orig, $new, $name, $product_id, $img_count) {
		...
		$this->db->query("INSERT INTO `oc_ebay_image_import` SET `image_original` = '" . $this->db->escape($orig) . "', `image_new` = '" . $this->db->escape($new) . "', `name` = '" . $this->db->escape($name) . "', `product_id` = '" . (int)$product_id . "', `imgcount` = '" . (int)$img_count . "'");
	}
