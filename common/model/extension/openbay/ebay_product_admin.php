<?php
class ModelExtensionOpenBayEbayProductAdmin extends Model {
	...
	public function repairLinks() {
		...
		foreach($sql->rows as $row) {
			$sql2 = $this->db->query("SELECT * FROM `oc_ebay_listing` WHERE `product_id` = '" . (int)$row['product_id'] . "' AND `status` = 1 ORDER BY `ebay_listing_id` DESC");

			if ($sql2->num_rows > 1) {
				$this->db->query("UPDATE `oc_ebay_listing` SET `status` = 0  WHERE `product_id` = '" . (int)$row['product_id'] . "'");
				$this->db->query("UPDATE `oc_ebay_listing` SET `status` = 1  WHERE `ebay_listing_id` = '" . (int)$sql2->row['ebay_listing_id'] . "'");
			}
		}
	}
