<?php
class ModelExtensionOpenBayEtsyProductAdmin extends Model{
	public function getStatus($product_id) {
		$query = $this->db->query("SELECT * FROM `oc_etsy_listing` WHERE `product_id` = '" . (int)$product_id . "' AND `status` = 1 LIMIT 1");
	...
	public function addLink($product_id, $etsy_item_id, $status_id = 0) {
		$this->db->query("INSERT INTO `oc_etsy_listing` SET `product_id` = '" . (int)$product_id . "', `etsy_item_id` = '" . $this->db->escape($etsy_item_id) . "', `status` = '" . (int)$status_id . "', `created`  = now()");
	}
