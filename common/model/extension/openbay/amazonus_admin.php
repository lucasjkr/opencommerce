<?php
class ModelExtensionOpenBayAmazonusAdmin extends Model {
	...
	public function uninstall() {
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_order`");
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_order_product`");
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_product2`");
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_product`");
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_product_link`");
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_product_unshipped`");
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_product_error`");
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_process`");
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_product_search`");
		$this->db->query("DROP TABLE IF EXISTS `oc_amazonus_listing_report`");

		$this->db->query("DELETE FROM `oc_setting` WHERE `code` = 'openbay_amazonus'");
  ...
  <MarketplaceIDs>';
	...
	public function deleteSaved($product_id, $var = '') {
		$this->db->query("DELETE FROM `oc_amazonus_product`
	...
	public function getProductStatus($product_id) {
		...
		if ($rows_uploaded == 0 && $rows_error > 0 && $rows_ok == 0) {
			$quick = $this->db->query("SELECT * FROM `oc_amazonus_product` WHERE `product_id` = " . (int)$product_id . " AND `version` = 3")->row;
	...
	public function getProductErrors($product_id, $version = 2) {
		...
		if (!empty($insertion_rows)) {
			foreach($insertion_rows as $insertion_row) {
				$error_rows = $this->db->query("SELECT * FROM `oc_amazonus_product_error` WHERE `sku` = '" . $this->db->escape($insertion_row['sku']) . "' AND `insertion_id` = '" . $this->db->escape($insertion_row['insertion_id']) . "'")->rows;
	...
	public function deleteProduct($product_id) {
		$this->db->query(
			"DELETE FROM `oc_amazonus_product`
			WHERE `product_id` = '" . (int)$product_id . "'");
	}

	public function linkProduct($amazonus_sku, $product_id, $var = '') {
		$count = $this->db->query("SELECT COUNT(*) as `count` FROM `oc_amazonus_product_link` WHERE `product_id` = '" . (int)$product_id . "' AND `amazonus_sku` = '" . $this->db->escape($amazonus_sku) . "' AND `var` = '" . $this->db->escape($var) . "' LIMIT 1")->row;

		if ($count['count'] == 0) {
			$this->db->query("INSERT INTO `oc_amazonus_product_link` SET `product_id` = '" . (int)$product_id . "', `amazonus_sku` = '" . $this->db->escape($amazonus_sku) . "', `var` = '" . $this->db->escape($var) . "'");
		}
	}

	public function removeProductLink($amazonus_sku) {
		$this->db->query(
			"DELETE FROM `oc_amazonus_product_link`
	...
	public function removeAdvancedErrors($product_id) {
		...
		foreach ($product_rows as $product) {
			$this->db->query(
				"DELETE FROM `oc_amazonus_product_error`
	...
	public function getProductLinks($product_id = 'all') {
		$query = "SELECT `apl`.`amazonus_sku`, `apl`.`product_id`, `pd`.`name` as `product_name`, `p`.`model`, `p`.`sku`, `apl`.`var`, '' as `combination`
			FROM `oc_amazonus_product_link` as `apl`
			LEFT JOIN `oc_product_description` as `pd`
	...
	private function productLinkExists($product_id, $var) {
		$link = $this->db->query("SELECT * FROM `oc_amazonus_product_link` WHERE `product_id` = " . (int)$product_id . " AND var = '" . $this->db->escape($var) . "'")->row;
	...
	public function updateAmazonusOrderTracking($order_id, $courier_id, $courier_from_list, $tracking_no) {
		$this->db->query("UPDATE `oc_amazonus_order` SET `courier_id` = '" . $this->db->escape($courier_id) . "', `courier_other` = " . (int)!$courier_from_list . ", `tracking_no` = '" . $this->db->escape($tracking_no) . "' WHERE `order_id` = " . (int)$order_id . "");
	}
	...
	public function getProductSearchTotal($data = array()) {
		$sql = "
			...
			FROM oc_product p
			LEFT JOIN oc_amazonus_product_search aps ON p.product_id = aps.product_id
			LEFT JOIN oc_amazonus_product_link apl ON p.product_id = apl.product_id
			LEFT JOIN oc_amazonus_product ap ON p.product_id = ap.product_id
