...
class ModelExtensionPaymentSecureTradingWs extends Model {
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM oc_zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_securetrading_ws_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
	...
	public function addMd($order_id, $md) {
		$this->db->query("INSERT INTO oc_securetrading_ws_order SET order_id = " . (int)$order_id . ", md = '" . $this->db->escape($md) . "', `created` = now(), `modified` = now()");
	}

	public function removeMd($md) {
		$this->db->query("DELETE FROM oc_securetrading_ws_order WHERE md = '" . $this->db->escape($md) . "'");
	}

	public function updateReference($order_id, $transaction_reference) {
		$this->db->query("UPDATE oc_securetrading_ws_order SET transaction_reference = '" . $this->db->escape($transaction_reference) . "' WHERE order_id = " . (int)$order_id);

		if ($this->db->countAffected() == 0) {
			$this->db->query("INSERT INTO oc_securetrading_ws_order SET order_id = " . (int)$order_id . ", transaction_reference = '" . $this->db->escape($transaction_reference) . "', `created` = now(), `modified` = now()");
		}
	}

	public function getOrderId($md) {
		$row = $this->db->query("SELECT order_id FROM oc_securetrading_ws_order WHERE md = '" . $this->db->escape($md) . "' LIMIT 1")->row;
	...
	public function confirmOrder($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		$this->db->query("UPDATE `oc_order` SET order_status_id = 0 WHERE order_id = " . (int)$order_id);
		...
		$this->db->query("UPDATE `oc_securetrading_ws_order` SET `settle_type` = '" . $this->config->get('payment_securetrading_ws_settle_status') . "', `modified` = now(), `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . $amount . "' WHERE order_id = " . (int)$order_info['order_id']);

		$this->db->query("INSERT INTO `oc_securetrading_ws_order_transaction` SET `securetrading_ws_order_id` = '" . (int)$securetrading_ws_order['securetrading_ws_order_id'] . "', `amount` = '" . $amount . "', type = '" . $trans_type . "',  `created` = now()");
	}
