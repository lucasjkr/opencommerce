...
class ModelExtensionPaymentSagePayServer extends Model {
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM `oc_zone_to_geo_zone` WHERE geo_zone_id = '" . (int)$this->config->get('payment_sagepay_server_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
	...
	public function getCards($customer_id) {
		...
		$query = $this->db->query("SELECT * FROM `oc_sagepay_server_card` WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function getCard($card_id, $token) {
		$qry = $this->db->query("SELECT * FROM oc_sagepay_server_card WHERE (card_id = '" . $this->db->escape($card_id) . "' OR token = '" . $this->db->escape($token) . "') AND customer_id = '" . (int)$this->customer->getId() . "'");
	...
	public function addCard($data) {
		$this->db->query("INSERT into `oc_sagepay_server_card` SET customer_id = '" . $this->db->escape((string)$data['customer_id']) . "', token = '" . $this->db->escape((string)$data['Token']) . "', digits = '" . $this->db->escape((string)$data['Last4Digits']) . "', expiry = '" . $this->db->escape((string)$data['ExpiryDate']) . "', type = '" . $this->db->escape((string)$data['CardType']) . "'");
	}

	public function deleteCard($card_id) {
		$this->db->query("DELETE FROM oc_sagepay_server_card WHERE card_id = '" . (int)$card_id . "'");
	}

	public function addOrder($order_info) {
		$this->db->query("DELETE FROM `oc_sagepay_server_order` WHERE `order_id` = '" . (int)$order_info['order_id'] . "'");
		
		$this->db->query("INSERT INTO `oc_sagepay_server_order` SET `order_id` = '" . (int)$order_info['order_id'] . "', `customer_id` = '" . (int)$this->customer->getId() . "', `VPSTxId` = '" . $this->db->escape($order_info['VPSTxId']) . "',  `VendorTxCode` = '" . $this->db->escape($order_info['VendorTxCode']) . "', `SecurityKey` = '" . $this->db->escape($order_info['SecurityKey']) . "', `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], false, false) . "'");
	}

	public function getOrder($order_id, $vpstx_id = null) {
		$qry = $this->db->query("SELECT * FROM `oc_sagepay_server_order` WHERE `order_id` = '" . (int)$order_id . "' OR `VPSTxId` = '" . $this->db->escape($vpstx_id) . "' LIMIT 1");
	...
	public function updateOrder($order_info, $vps_txn_id, $tx_auth_no) {
		$this->db->query("UPDATE `oc_sagepay_server_order` SET `VPSTxId` = '" . $this->db->escape($vps_txn_id) . "', `TxAuthNo` = '" . $this->db->escape($tx_auth_no) . "' WHERE `order_id` = '" . (int)$order_info['order_id'] . "'");
	}

	public function deleteOrder($order_id) {
		$this->db->query("DELETE FROM `oc_sagepay_server_order` WHERE order_id = '" . (int)$order_id . "'");
		$this->db->query("DELETE FROM `oc_order_recurring` WHERE order_id = '" . (int)$order_id . "'");
	}

	public function addTransaction($sagepay_server_order_id, $type, $order_info) {
		$this->db->query("INSERT INTO `oc_sagepay_server_order_transaction` SET `sagepay_server_order_id` = '" . (int)$sagepay_server_order_id . "', `type` = '" . $this->db->escape($type) . "', `amount` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], false, false) . "'");
	}

	private function getTransactions($sagepay_server_order_id) {
		$qry = $this->db->query("SELECT * FROM `oc_sagepay_server_order_transaction` WHERE `sagepay_server_order_id` = '" . (int)$sagepay_server_order_id . "'");
	...
	public function getRecurringOrders($order_id) {
		$query = $this->db->query("SELECT * FROM `oc_order_recurring` WHERE order_id = '" . (int)$order_id . "'");
		return $query->rows;
	}
	...
	private function addRecurringOrder($order_id, $response_data, $order_recurring_id, $trial_end, $subscription_end) {
		$this->db->query("INSERT INTO `oc_sagepay_server_order_recurring` SET `order_id` = '" . (int)$order_id . "', `order_recurring_id` = '" . (int)$order_recurring_id . "', `VPSTxId` = '" . $this->db->escape($response_data['VPSTxId']) . "', `VendorTxCode` = '" . $this->db->escape($response_data['VendorTxCode']) . "', `SecurityKey` = '" . $this->db->escape($response_data['SecurityKey']) . "', `TxAuthNo` = '" . $this->db->escape($response_data['TxAuthNo']) . "', `next_payment` = now(), `trial_end` = '" . $trial_end . "', `subscription_end` = '" . $subscription_end . "', `currency_code` = '" . $this->db->escape($response_data['Currency']) . "', `total` = '" . $this->currency->format($response_data['Amount'], $response_data['Currency'], false, false) . "'");
	}

	private function updateRecurringOrder($order_recurring_id, $next_payment) {
		$this->db->query("UPDATE `oc_sagepay_server_order_recurring` SET `next_payment` = '" . $next_payment . "' WHERE `order_recurring_id` = '" . (int)$order_recurring_id . "'");
	}

	private function getRecurringOrder($order_recurring_id) {
		$qry = $this->db->query("SELECT * FROM oc_sagepay_server_order_recurring WHERE order_recurring_id = '" . (int)$order_recurring_id . "'");
		return $qry->row;
	}

	private function addRecurringTransaction($order_recurring_id, $response_data, $type) {
		$this->db->query("INSERT INTO `oc_order_recurring_transaction` SET `order_recurring_id` = '" . (int)$order_recurring_id . "', `amount` = '" . (float)$response_data['Amount'] . "', `type` = '" . (int)$type . "', `reference` = '" . $this->db->escape($response_data['VendorTxCode']) . "'");
	}
	...
	private function getProfiles() {
		...
		$sql = "
			...
			FROM `oc_order_recurring` `or`
			JOIN `oc_order` `o` USING(`order_id`)
	...
	private function getProfile($order_recurring_id) {
		$qry = $this->db->query("SELECT * FROM oc_order_recurring WHERE order_recurring_id = " . (int)$order_recurring_id);
		return $qry->row;
	}

	public function updateCronJobRunTime() {
		$this->db->query("DELETE FROM `oc_setting` WHERE `code` = 'sagepay_server' AND `key` = 'payment_sagepay_server_last_cron_job_run'");
		$this->db->query("INSERT INTO `oc_setting` (`store_id`, `code`, `key`, `value`, `serialized`) VALUES (0, 'sagepay_server', 'payment_sagepay_server_last_cron_job_run', NOW(), 0)");
	}
