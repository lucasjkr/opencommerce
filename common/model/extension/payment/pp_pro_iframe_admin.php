...
class ModelExtensionPaymentPPProIframeAdmin extends Model {
	...
	public function uninstall() {
		$this->db->query("DROP TABLE IF EXISTS `oc_paypal_iframe_order_transaction`;");
		$this->db->query("DROP TABLE IF EXISTS `oc_paypal_iframe_order`;");
	}

	private function getTransactions($paypal_iframe_order_id) {
		$qry = $this->db->query("SELECT `ot`.*, ( SELECT count(`ot2`.`paypal_iframe_order_id`) FROM `oc_paypal_iframe_order_transaction` `ot2` WHERE `ot2`.`parent_id` = `ot`.`transaction_id` ) AS `children` FROM `oc_paypal_iframe_order_transaction` `ot` WHERE `paypal_iframe_order_id` = '" . (int)$paypal_iframe_order_id . "'");
	...
	public function getTotalCaptured($paypal_iframe_order_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `oc_paypal_iframe_order_transaction` WHERE `paypal_iframe_order_id` = '" . (int)$paypal_iframe_order_id . "' AND `pending_reason` != 'authorization' AND (`payment_status` = 'Partially-Refunded' OR `payment_status` = 'Completed' OR `payment_status` = 'Pending') AND `transaction_entity` = 'payment'");

		return $qry->row['amount'];
	}

	public function getTotalRefunded($paypal_iframe_order_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `oc_paypal_iframe_order_transaction` WHERE `paypal_iframe_order_id` = '" . (int)$paypal_iframe_order_id . "' AND `payment_status` = 'Refunded'");

		return $qry->row['amount'];
	}

	public function getTotalRefundedTransaction($transaction_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `oc_paypal_iframe_order_transaction` WHERE `parent_id` = '" . $this->db->escape($transaction_id) . "' AND `payment_type` = 'refund'");

		return $qry->row['amount'];
	}

	public function getOrder($order_id) {
		$qry = $this->db->query("SELECT * FROM `oc_paypal_iframe_order` WHERE `order_id` = '" . (int)$order_id . "' LIMIT 1");
	...
	public function updateOrder($capture_status, $order_id) {
		$this->db->query("UPDATE `oc_paypal_iframe_order` SET `capture_status` = '" . $this->db->escape($capture_status) . "' WHERE `order_id` = '" . (int)$order_id . "'");
	}
	...
	public function getOrderId($transaction_id) {
		$qry = $this->db->query("SELECT `o`.`order_id` FROM `oc_paypal_iframe_order_transaction` `ot` LEFT JOIN `oc_paypal_iframe_order` `o`  ON `o`.`paypal_iframe_order_id` = `ot`.`paypal_iframe_order_id`  WHERE `ot`.`transaction_id` = '" . $this->db->escape($transaction_id) . "' LIMIT 1");
	...
	public function updateRefundTransaction($transaction_id, $transaction_type) {
		$this->db->query("UPDATE `oc_paypal_iframe_order_transaction` SET `payment_status` = '" . $this->db->escape($transaction_type) . "' WHERE `transaction_id` = '" . $this->db->escape($transaction_id) . "' LIMIT 1");
	}
