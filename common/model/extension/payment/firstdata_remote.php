...
class ModelExtensionPaymentFirstdataRemote extends Model {
	...
	public function addOrder($order_info, $capture_result) {
		...
		$this->db->query("INSERT INTO `oc_firstdata_remote_order` SET `order_id` = '" . (int)$order_info['order_id'] . "', `order_ref` = '" . $this->db->escape($capture_result['order_id']) . "', `authcode` = '" . $this->db->escape($capture_result['approval_code']) . "', `tdate` = '" . $this->db->escape($capture_result['t_date']) . "', `capture_status` = '" . (int)$settle_status . "', `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false) . "'");
	...
	public function addTransaction($firstdata_remote_order_id, $type, $order_info = []) {
		...
		$this->db->query("INSERT INTO `oc_firstdata_remote_order_transaction` SET `firstdata_remote_order_id` = '" . (int)$firstdata_remote_order_id . "', `type` = '" . $this->db->escape($type) . "', `amount` = '" . (float)$amount . "'");
	}
	...
	public function getStoredCards() {
		...
		$query = $this->db->query("SELECT * FROM oc_firstdata_remote_card WHERE customer_id = '" . (int)$customer_id . "'");

		return $query->rows;
	}

	public function storeCard($token, $customer_id, $type, $month, $year, $digits) {
		$this->db->query("INSERT INTO `oc_firstdata_remote_card` SET `customer_id` = '" . (int)$customer_id . "', `token` = '" . $this->db->escape($token) . "', `card_type` = '" . $this->db->escape($type) . "', `expire_month` = '" . $this->db->escape($month) . "', `expire_year` = '" . $this->db->escape($year) . "', `digits` = '" . $this->db->escape($digits) . "'");
	}
}