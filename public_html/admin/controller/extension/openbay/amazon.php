...
class ControllerExtensionOpenbayAmazon extends Controller {
	public function install() {
		...
		$this->model_user_user_group_admin->addPermission($this->user->getGroupId(), 'access', 'extension/openbay/amazon_listing');
		$this->model_user_user_group_admin->addPermission($this->user->getGroupId(), 'modify', 'extension/openbay/amazon_listing');
		$this->model_user_user_group_admin->addPermission($this->user->getGroupId(), 'access', 'extension/openbay/amazon_product');
		$this->model_user_user_group_admin->addPermission($this->user->getGroupId(), 'modify', 'extension/openbay/amazon_product');

		$this->model_extension_openbay_amazon_admin->install();
	}
	...
	public function uninstall() {
		...
		$this->model_extension_openbay_amazon_admin->uninstall();
	...
	public function settings() {
		...
		$settings = $this->model_setting_setting_admin->getSetting('openbay_amazon');

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->post['openbay_amazon_orders_marketplace_ids'])) {
				$this->request->post['openbay_amazon_orders_marketplace_ids'] = [];
			}

			$settings = array_merge($settings, $this->request->post);
			$this->model_setting_setting_admin->editSetting('openbay_amazon', $settings);
			...
			if (!empty($this->request->post['openbay_amazon_token']) && !empty($this->request->post['openbay_amazon_encryption_key']) && !empty($this->request->post['openbay_amazon_encryption_iv'])) {
                $this->model_extension_openbay_amazon_admin->verifyConfig($settings);
            }
	...
	public function savedListings() {
		...
		$data['user_token'] = $this->session->data['user_token'];
		$this->load->model('extension/openbay/amazon_admin');
		$saved_products = $this->model_extension_openbay_amazon_admin->getSavedProducts();
	...
	public function addLink() {
		if (isset($this->request->get['product_id']) && isset($this->request->get['amazon_sku'])) {
			...
			$this->model_extension_openbay_amazon_admin->linkProduct($amazon_sku, $product_id, $var);
	...
	public function deleteLink() {
		if (isset($this->request->get['amazon_sku'])) {
			...
			$this->model_extension_openbay_amazon_admin->removeProductLink($amazon_sku);
	...
	public function getLinks() {
		...
		$json = json_encode($this->model_extension_openbay_amazon_admin->getProductLinks());
	...
	public function getUnlinked() {
		...
		$json = json_encode($this->model_extension_openbay_amazon_admin->getUnlinkedProducts());
	...
	public function doBulkSearch() {
		...
		if (!empty($this->request->post['product_ids'])) {
			foreach ($this->request->post['product_ids'] as $product_id) {
				$product = $this->model_catalog_product_admin->getProduct($product_id);
	...
	public function bulkListProducts() {
		...
		if ($bulk_listing_status) {
			...
			$results = $this->model_extension_openbay_amazon_admin->getProductSearch($filter);
			$product_total = $this->model_extension_openbay_amazon_admin->getProductSearchTotal($filter);
			...
			foreach ($results as $result) {
				...
				if ($result['data']) {
					foreach ($result['data'] as $search_result) {
						$link = $this->model_extension_openbay_amazon_admin->getAsinLink($search_result['asin'], $result['marketplace']);
	...
	public function bulkLinking() {
		...
		$total_linked = $this->model_extension_openbay_amazon_admin->getTotalUnlinkedItemsFromReport($marketplace_code);
		...
		$results = $this->model_extension_openbay_amazon_admin->getUnlinkedItemsFromReport($marketplace_code, $linked_item_limit, $linked_item_page);
		...
		foreach ($results as $result) {
			$products[] = array(
				'asin' => $result['asin'],
				'href_amazon' => $this->model_extension_openbay_amazon_admin->getAsinLink($result['asin'], $marketplace_code),
	...
	public function loadListingReport() {
		...
		$this->model_extension_openbay_amazon_admin->deleteListingReports($marketplace);
	...
	public function doBulkLinking() {
		...
		if (!empty($links)) {
			foreach ($links as $link) {
				$this->model_extension_openbay_amazon_admin->linkProduct($link['amazon_sku'], $link['product_id'], $link['sku']);
			}

			$this->model_extension_openbay_amazon_admin->updateAmazonSkusQuantities($skus);
		}
