...
class ControllerExtensionOpenbayAmazonListing extends Controller {
	public function create() {
		...
		if (isset($this->request->get['product_id'])) {
			...
			$listing_status = $this->model_extension_openbay_amazon_admin->getProductStatus($this->request->get['product_id']);
		...
		if ($listing_status == 'error_quick') {
			$data['listing_errors'] = $this->model_extension_openbay_amazon_admin->getProductErrors($product_info['product_id'], 3);
		}
	...
	public function edit() {
		...
		$status = $this->model_extension_openbay_amazon_admin->getProductStatus($product_id);
		...
		$data['product_links'] = $this->model_extension_openbay_amazon_admin->getProductLinks($product_id);
		$data['url_return']  = $this->url->link('marketplace/openbay/items', 'user_token=' . $this->session->data['user_token'] . $url, true);
	...
	public function createNew() {
		...
		$this->load->model('extension/openbay/amazon_admin');
		$this->model_extension_openbay_amazon_admin->deleteProduct($product_id);
		$this->response->redirect($this->url->link('extension/openbay/amazon_listing/create', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $product_id . $url, true));
	}
	...
	public function deleteLinks() {
		...
		$links = $this->model_extension_openbay_amazon_admin->getProductLinks($product_id);
		foreach ($links as $link) {
			$this->model_extension_openbay_amazon_admin->removeProductLink($link['amazon_sku']);
		}
		$this->model_extension_openbay_amazon_admin->deleteProduct($product_id);
