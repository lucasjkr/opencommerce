...
class ModelCustomerCustomerApproval extends Model {
	public function getCustomerApprovals($data = array()) {
		$sql = "SELECT *, CONCAT(c.`firstname`, ' ', c.`lastname`) AS name, cgd.`name` AS customer_group, ca.`type` FROM `oc_customer_approval` ca LEFT JOIN `oc_customer` c ON (ca.`customer_id` = c.`customer_id`) LEFT JOIN `oc_customer_group_description` cgd ON (c.`customer_group_id` = cgd.`customer_group_id`) WHERE cgd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getCustomerApproval($customer_approval_id) {
		$query = $this->db->query("SELECT * FROM `oc_customer_approval` WHERE `customer_approval_id` = '" . (int)$customer_approval_id . "'");
	...
	public function approveCustomer($customer_id) {
		$this->db->query("UPDATE `oc_customer` SET status = '1' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function approveAffiliate($customer_id) {
		$this->db->query("UPDATE `oc_customer_affiliate` SET status = '1' WHERE customer_id = '" . (int)$customer_id . "'");
		$this->db->query("DELETE FROM `oc_customer_approval` WHERE customer_id = '" . (int)$customer_id . "' AND `type` = 'affiliate'");
	}
