...
class ModelLocalisationReturnStatus extends Model {
	public function addReturnStatus($data) {
		foreach ($data['return_status'] as $language_id => $value) {
			if (isset($return_status_id)) {
				$this->db->query("INSERT INTO oc_return_status SET return_status_id = '" . (int)$return_status_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
			} else {
				$this->db->query("INSERT INTO oc_return_status SET language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
	...
	public function editReturnStatus($return_status_id, $data) {
		$this->db->query("DELETE FROM oc_return_status WHERE return_status_id = '" . (int)$return_status_id . "'");
	...
	public function getReturnStatus($return_status_id) {
		$query = $this->db->query("SELECT * FROM oc_return_status WHERE return_status_id = '" . (int)$return_status_id . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getReturnStatuses($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM oc_return_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'";
			...
			if (!$return_status_data) {
				$query = $this->db->query("SELECT return_status_id, name FROM oc_return_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name");
	...
	public function getTotalReturnStatuses() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM oc_return_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");
