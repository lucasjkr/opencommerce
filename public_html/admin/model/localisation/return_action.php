...
class ModelLocalisationReturnAction extends Model {
	public function addReturnAction($data) {
		foreach ($data['return_action'] as $language_id => $value) {
			if (isset($return_action_id)) {
				$this->db->query("INSERT INTO oc_return_action SET return_action_id = '" . (int)$return_action_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
			} else {
				$this->db->query("INSERT INTO oc_return_action SET language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
	...
	public function editReturnAction($return_action_id, $data) {
		$this->db->query("DELETE FROM oc_return_action WHERE return_action_id = '" . (int)$return_action_id . "'");
	...
	public function getReturnAction($return_action_id) {
		$query = $this->db->query("SELECT * FROM oc_return_action WHERE return_action_id = '" . (int)$return_action_id . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getReturnActions($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM oc_return_action WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'";
			...
			if (!$return_action_data) {
				$query = $this->db->query("SELECT return_action_id, name FROM oc_return_action WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name");
	...
	public function getTotalReturnActions() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM oc_return_action WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");
