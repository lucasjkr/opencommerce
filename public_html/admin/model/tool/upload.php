...
class ModelToolUpload extends Model {
	public function addUpload($name, $filename) {
		...
		$this->db->query("INSERT INTO `oc_upload` SET `name` = '" . $this->db->escape($name) . "', `filename` = '" . $this->db->escape($filename) . "', `code` = '" . $this->db->escape($code) . "'");

		return $code;
	}
		
	public function deleteUpload($upload_id) {
		$this->db->query("DELETE FROM oc_upload WHERE upload_id = '" . (int)$upload_id . "'");
	}

	public function getUpload($upload_id) {
		$query = $this->db->query("SELECT * FROM `oc_upload` WHERE upload_id = '" . (int)$upload_id . "'");
