...
class ModelMarketingAffiliate extends Model {
	...
	public function getAffiliate($customer_id) {
		$query = $this->db->query("SELECT DISTINCT *, CONCAT(c.firstname, ' ', c.lastname) AS customer FROM oc_customer_affiliate ca LEFT JOIN `oc_customer` c ON (ca.customer_id = c.customer_id) WHERE ca.customer_id = '" . (int)$customer_id . "'");

		return $query->row;
	}

	public function getAffliateByTracking($tracking) {
		$query = $this->db->query("SELECT * FROM oc_customer_affiliate WHERE tracking = '" . $this->db->escape($tracking) . "'");
	...
	public function getReports($customer_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT ip, store_id, country, date_added FROM oc_customer_affiliate_report WHERE customer_id = '" . (int)$customer_id . "' ORDER BY date_added ASC LIMIT " . (int)$start . "," . (int)$limit);

		return $query->rows;
	}

	public function getTotalReports($customer_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM oc_customer_affiliate_report WHERE customer_id = '" . (int)$customer_id . "'");
