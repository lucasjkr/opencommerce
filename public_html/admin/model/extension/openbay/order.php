...
class ModelExtensionOpenBayOrder extends Model {
	public function getTotalOrders($data = array()) {
		...
		if ($this->config->get('ebay_status')) {
			$sql .= " LEFT JOIN oc_ebay_order eo ON o.order_id = eo.order_id ";
		} else {
			$sql .= " JOIN (SELECT NULL AS order_id) eo ";
		}

		if ($this->config->get('openbay_amazon_status')) {
			$sql .= " LEFT JOIN oc_amazon_order ao ON o.order_id = ao.order_id ";
		} else {
			$sql .= " JOIN (SELECT NULL AS order_id) ao ";
		}

		if ($this->config->get('openbay_amazonus_status')) {
			$sql .= " LEFT JOIN oc_amazonus_order auso ON o.order_id = auso.order_id ";
		} else {
			$sql .= " JOIN (SELECT NULL AS order_id) auso ";
		}

		if ($this->config->get('etsy_status')) {
			$sql .= " LEFT JOIN oc_etsy_order eto ON o.order_id = eto.order_id ";
		} else {
	...
	public function getOrders($data = array()) {
		...
		if ($this->config->get('ebay_status')) {
			$sql .= " LEFT JOIN oc_ebay_order eo ON eo.order_id = o.order_id";
		} else {
			$sql .= " JOIN (SELECT NULL AS order_id) eo ";
		}

		if ($this->config->get('openbay_amazon_status')) {
			$sql .= " LEFT JOIN oc_amazon_order ao ON ao.order_id = o.order_id ";
		} else {
			$sql .= " JOIN (SELECT NULL AS order_id) ao ";
		}

		if ($this->config->get('openbay_amazonus_status')) {
			$sql .= " LEFT JOIN oc_amazonus_order auso ON auso.order_id = o.order_id ";
		} else {
			$sql .= " JOIN (SELECT NULL AS order_id) auso ";
		}

		if ($this->config->get('etsy_status')) {
			$sql .= " LEFT JOIN oc_etsy_order eto ON eto.order_id = o.order_id ";
		} else {
			$sql .= " JOIN (SELECT NULL AS order_id) eto ";
		}
