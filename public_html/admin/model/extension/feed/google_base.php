...
class ModelExtensionFeedGoogleBase extends Model {
    ...
    public function import($string) {
        ...
        foreach ($lines as $line) {
			if (substr($line, 0, 1) != '#') {
	            ...
	            if (isset($part[1])) {
	                $this->db->query("INSERT INTO oc_google_base_category SET google_base_category_id = '" . (int)$part[0] . "', name = '" . $this->db->escape($part[1]) . "'");
	            }
			}
        }
    }
    ...
    public function getGoogleBaseCategories($data = array()) {
        $sql = "SELECT * FROM `oc_google_base_category` WHERE name LIKE '%" . $this->db->escape((string)$data['filter_name']) . "%' ORDER BY name ASC";
	...
	public function addCategory($data) {
		$this->db->query("DELETE FROM oc_google_base_category_to_category WHERE category_id = '" . (int)$data['category_id'] . "'");

		$this->db->query("INSERT INTO oc_google_base_category_to_category SET google_base_category_id = '" . (int)$data['google_base_category_id'] . "', category_id = '" . (int)$data['category_id'] . "'");
	}

	public function deleteCategory($category_id) {
		$this->db->query("DELETE FROM oc_google_base_category_to_category WHERE category_id = '" . (int)$category_id . "'");
	}

    public function getCategories($data = array()) {
        $sql = "SELECT google_base_category_id, (SELECT name FROM `oc_google_base_category` gbc WHERE gbc.google_base_category_id = gbc2c.google_base_category_id) AS google_base_category, category_id, (SELECT name FROM `oc_category_description` cd WHERE cd.category_id = gbc2c.category_id AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS category FROM `oc_google_base_category_to_category` gbc2c ORDER BY google_base_category ASC";
