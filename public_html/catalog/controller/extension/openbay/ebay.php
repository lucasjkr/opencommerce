			...
			if(isset($data['secret']) && $secret == $data['secret'] && $active == 1) {
				if($data['action'] == 'ItemUnsold') {
					...
					if($product_id != false) {
						...
						if(!empty($rules)) {
							...
							$this->db->query("INSERT INTO `oc_ebay_listing_pending` SET `ebay_item_id` = '" . $this->db->escape((string)$data['itemId']) . "', `product_id` = '" . (int)$product_id . "', `key` = '" . $this->db->escape((string)$data['key']) . "'");
				...
				if($data['action'] == 'ItemListed') {
					...
					if($product_id != false) {
						...
						$this->db->query("DELETE FROM `oc_ebay_listing_pending` WHERE `key` = '" . $this->db->escape((string)$data['key']) . "' LIMIT 1");
