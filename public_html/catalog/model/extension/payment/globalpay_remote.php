...
class ModelExtensionPaymentGlobalpayRemote extends Model {
	...
	public function addTransaction($globalpay_remote_order_id, $type, $order_info) {
		$this->db->query("INSERT INTO `oc_globalpay_remote_order_transaction` SET `globalpay_remote_order_id` = '" . (int)$globalpay_remote_order_id . "', `date_added` = now(), `type` = '" . $this->db->escape($type) . "', `amount` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false) . "'");
	}
