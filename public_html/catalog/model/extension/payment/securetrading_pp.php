...
class ModelExtensionPaymentSecureTradingPp extends Model {
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM oc_zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_securetrading_pp_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
	...
	public function confirmOrder($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		$this->db->query("UPDATE `oc_securetrading_pp_order` SET `settle_type` = '" . $this->config->get('payment_securetrading_pp_settle_status') . "', `modified` = now(), `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . $amount . "' WHERE order_id = " . (int)$order_info['order_id']);

		$this->db->query("INSERT INTO `oc_securetrading_pp_order_transaction` SET `securetrading_pp_order_id` = '" . (int)$securetrading_pp_order['securetrading_pp_order_id'] . "', `amount` = '" . $amount . "', type = '" . $trans_type . "',  `created` = now()");
	...
	public function getCountry($iso_code_2) {
		return $this->db->query("SELECT * FROM oc_country WHERE LOWER(iso_code_2) = '" . $this->db->escape($iso_code_2) . "'")->row;
	}
