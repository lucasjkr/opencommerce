...
class ModelExtensionPaymentDivido extends Model {
	...
	public function getProductSettings($product_id) {
		return $this->db->query("SELECT `display`, `plans` FROM `oc_divido_product` WHERE `product_id` = '" . (int)$product_id . "'")->row;
	}
	...
	public function saveLookup($order_id, $salt, $proposal_id = null, $application_id = null, $deposit_amount = null) {
		...
		$query_get_lookup = "SELECT `application_id` from `oc_divido_lookup` WHERE order_id = " . $order_id;
		...
		if ($result_get_lookup->num_rows == 0) {
			...
			$query_upsert = "INSERT INTO `oc_divido_lookup` (`order_id`, `salt`, `proposal_id`, `application_id`, `deposit_amount`) VALUES (" . $order_id . ", '" . $salt . "', " . $proposal_id . ", " . $application_id . ", " . $deposit_amount . ")";
		} else {
			$query_upsert = "UPDATE `oc_divido_lookup` SET `salt` = '" . $salt . "'";
	...
	public function getLookupByOrderId($order_id) {
		return $this->db->query("SELECT * FROM `oc_divido_lookup` WHERE `order_id` = " . $order_id);
	}
