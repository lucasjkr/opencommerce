...
class ModelExtensionPaymentFirstdataRemote extends Model {
	...
	public function addTransaction($firstdata_remote_order_id, $type, $order_info = array()) {
		...
		$this->db->query("INSERT INTO `oc_firstdata_remote_order_transaction` SET `firstdata_remote_order_id` = '" . (int)$firstdata_remote_order_id . "', `date_added` = now(), `type` = '" . $this->db->escape($type) . "', `amount` = '" . (float)$amount . "'");
	}
	...
	public function storeCard($token, $customer_id, $type, $month, $year, $digits) {
		$this->db->query("INSERT INTO `oc_firstdata_remote_card` SET `customer_id` = '" . (int)$customer_id . "', `date_added` = now(), `token` = '" . $this->db->escape($token) . "', `card_type` = '" . $this->db->escape($type) . "', `expire_month` = '" . $this->db->escape($month) . "', `expire_year` = '" . $this->db->escape($year) . "', `digits` = '" . $this->db->escape($digits) . "'");
	}
}