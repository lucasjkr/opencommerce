...
class ModelExtensionOpenBayAmazonusOrder extends Model {
  ...
  <MerchantOrderId>$order_id</MerchantOrderId>
	...
	public function getProductId($sku) {
		$row = $this->db->query("SELECT `product_id` FROM `oc_amazonus_product_link` WHERE `amazonus_sku` = '" . $this->db->escape($sku) . "'")->row;
	...
	public function getProductVar($sku) {
		$row = $this->db->query("SELECT `var` FROM `oc_amazonus_product_link` WHERE `amazonus_sku` = '" . $this->db->escape($sku) . "'")->row;
	...
	public function decreaseProductQuantity($product_id, $delta, $var = '') {
		...
		if ($var == '') {
			$this->db->query("UPDATE `oc_product` SET `quantity` = GREATEST(`quantity` - '" . (int)$delta . "', 0) WHERE `product_id` = '" . (int)$product_id . "' AND `subtract` = '1'");
		} else {
			$this->db->query("UPDATE `oc_product_option_variant` SET `stock` = GREATEST(`stock` - '" . (int)$delta . "', 0) WHERE `product_id` = '" . (int)$product_id . "' AND `sku` = '" . $this->db->escape($var) . "' AND `subtract` = '1'");
		}
	}
	...
	public function getCountryId($country_code) {
		$row = $this->db->query("SELECT `country_id` FROM `oc_country` WHERE LOWER(`iso_code_2`) = '" . $this->db->escape(strtolower($country_code)) . "'")->row;
	...
	public function getZoneId($zone_name) {
		$row = $this->db->query("SELECT `zone_id` FROM `oc_zone` WHERE LOWER(`name`) = '" . $this->db->escape(strtolower($zone_name)) . "'")->row;
	...
	public function updateOrderStatus($order_id, $status_id) {
		$this->db->query("INSERT INTO `oc_order_history` (`order_id`, `order_status_id`, `notify`, `comment`, `date_added`) VALUES (" . (int)$order_id . ", " . (int)$status_id . ", 0, '', NOW())");

		$this->db->query("UPDATE `oc_order` SET `order_status_id` = " . (int)$status_id . " WHERE `order_id` = " . (int)$order_id);
	}

	public function addAmazonusOrder($order_id, $amazonus_order_id) {
		$this->db->query("INSERT INTO `oc_amazonus_order` (`order_id`, `amazonus_order_id`) VALUES (" . (int)$order_id . ", '" . $this->db->escape($amazonus_order_id) . "')");
	}
	...
	public function addAmazonusOrderProducts($order_id, $data) {
		foreach ($data as $sku => $order_item_id) {
			...
			$row = $this->db->query("SELECT `order_product_id` FROM `oc_order_product` WHERE `model` = '" . $this->db->escape($sku) . "' AND `order_id` = " . (int)$order_id . "LIMIT 1")->row;
			...
			$this->db->query("INSERT INTO `oc_amazonus_order_product` (`order_product_id`, `amazonus_order_item_id`) VALUES (" . (int)$order_product_id . ", '" . $this->db->escape($order_item_id) . "')");
		}
	}

	public function getOrderId($amazonus_order_id) {
		$row = $this->db->query("SELECT `order_id` FROM `oc_amazonus_order` WHERE `amazonus_order_id` = '" . $this->db->escape($amazonus_order_id) . "' LIMIT 1")->row;
	...
	public function getOrderStatus($order_id) {
		$row = $this->db->query("SELECT `order_status_id` FROM `oc_order` WHERE `order_id` = " . (int)$order_id)->row;
	...
	public function getAmazonusOrderId($order_id) {
		$row = $this->db->query("SELECT `amazonus_order_id` FROM `oc_amazonus_order` WHERE `order_id` = " . (int)$order_id . " LIMIT 1")->row;
