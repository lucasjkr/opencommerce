...
class ModelExtensionOpenBayAmazonOrder extends Model {
  ...
  <MerchantOrderId>$order_id</MerchantOrderId>
	...
	public function getProductId($sku) {
		$row = $this->db->query("SELECT `product_id` FROM `oc_amazon_product_link` WHERE `amazon_sku` = '" . $this->db->escape($sku) . "'")->row;
	...
	public function getProductVar($sku) {
		$row = $this->db->query("SELECT `var` FROM `oc_amazon_product_link` WHERE `amazon_sku` = '" . $this->db->escape($sku) . "'")->row;
	...
	public function addAmazonOrder($order_id, $amazon_order_id) {
		$this->db->query("INSERT INTO `oc_amazon_order` (`order_id`, `amazon_order_id`) VALUES (" . (int)$order_id . ", '" . $this->db->escape($amazon_order_id) . "')");
	}
	...
	public function addAmazonOrderProducts($order_id, $data) {
		foreach ($data as $sku => $order_item_id) {
			...
			$this->db->query("INSERT INTO `oc_amazon_order_product` (`order_product_id`, `amazon_order_item_id`) VALUES (" . (int)$order_product_id . ", '" . $this->db->escape($order_item_id) . "')");
		}
	}

	public function getOrderId($amazon_order_id) {
		$row = $this->db->query("SELECT `order_id` FROM `oc_amazon_order` WHERE `amazon_order_id` = '" . $this->db->escape($amazon_order_id) . "' LIMIT 1")->row;
	...
	public function getAmazonOrderId($order_id) {
		$row = $this->db->query("SELECT `amazon_order_id` FROM `oc_amazon_order` WHERE `order_id` = " . (int)$order_id . " LIMIT 1")->row;
